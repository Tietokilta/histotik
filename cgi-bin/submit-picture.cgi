...
if [ "X$URL" = "X" ] ; then
    ...
    echo "            <P>"
    echo "            "Palaa tästä \<A href=\"$HTTP_REFERER\"\>edelliselle sivulle.\</A\>
    cat $HTMLTAIL
    exit
fi

if [ "X$NIMI" = "X" ] ; then
    echo "            <H1>Lähettäjän nimi puuttuu!</H1>"
    echo "            <P>"
    echo "            "Palaa tästä \<A href=\"$HTTP_REFERER\"\>edelliselle sivulle.\</A\>
    cat $HTMLTAIL
    exit
...
if [ $VUOSI -eq `date +'%Y'` ] ; then
	...
	echo "            <H1>Antamasi päivämäärä on tulevaisuudessa!</H1>"
	echo "            <P>"
	echo "            "Palaa tästä \<A HREF=\"$HTTP_REFERER\"\>edelliselle sivulle.\</A\>
	cat $HTMLTAIL
	exit
...
NIMI=`echo $NIMI | sed 's/%E4/ä/g; s/%F6/ö/g; s/%E5/å/g; s/%C4/Ä/g; s/%D6/Ö/g; s/%C5/Å/g; s/%40/@/g; s/%3A/:/g; s/%2F/\\//g; s/%2C/,/g'`

KUVAUS=`echo $KUVAUS | sed 's/%E4/ä/g; s/%F6/ö/g; s/%E5/å/g; s/%C4/Ä/g; s/%D6/Ö/g; s/%C5/Å/g; s/%40/@/g; s/%3A/:/g; s/%2F/\\//g; s/%2C/,/g'`
...
# kuukaudelle laitetut kuvat eivät mene päällekkäin
...
if /usr/bin/wget -q $URL -O $PICDIR/$VUOSI/pic-$KUUKAUSI-$NUM.jpg; then 
	...
	# nyt saavutaan kriittisimpään sektioon, hoidetaan simppelillä lukolla
...
#		cat $HTMLTAIL | sed s/"\.\.\/"/"..\/..\/"/g >> $PICDIR/$VUOSI/$i.html
	...
	echo "            "Palaa tästä \<A href=\"$HTTP_REFERER\"\>edelliselle sivulle.\</A\>
	echo "            <P>"
        echo "            "\<A href=\"$PICDIR/$VUOSI/\"\>Tästä\</A\> pääset vuoden $VUOSI kuvagalleriaan. 
	cat $HTMLTAIL
else
	echo "            <H1>Kuvan vastaanotossa ongelmia. Tarkista tiedot ja yritä uudelleen.</H1>"
	echo "            <P>"
	echo "            "Palaa tästä \<A href=\"$HTTP_REFERER\"\>edelliselle sivulle.\</A\>
	cat $HTMLTAIL
